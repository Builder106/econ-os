...
# 1. Add triple-slash reference
text = text.replace('// @ts-check', '/// <reference path="./global.d.ts" />\n// @ts-check')

# 2. Methods and functions missing parameter types
...
# 3. Add types to array / variable initialization
...
sub("""const header = win.querySelector('.window-header');
        header.addEventListener('mousedown', (/** @type {any} */ e) => this.startDragging(e, win));""", """const header = win.querySelector('.window-header');
        if (header) header.addEventListener('mousedown', (/** @type {any} */ e) => this.startDragging(e, win));""")


sub("const chart = new Chart(ctx, {", "const chart = new window.Chart(ctx, {")
